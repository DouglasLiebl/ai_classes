...
class TrainingParameters(BaseModel):
    ...
    rgb_ranges: Optional[Dict[str, Any]] = None

@app.post("/cnn/create-upload-session")
...
@app.post("/cnn/upload-batch/{session_id}")
async def upload_batch(
    ...
    try:
        ...
        updated_metadata = BatchUploadManager.add_files(session_id, class_name, processed_files)
...
@app.post("/cnn/train-from-session/{session_id}")
async def train_from_session(
    ...
    try:
        ...
        parsed_rgb_ranges = params.rgb_ranges
        if parsed_rgb_ranges:
            print(f"RGB ranges detected for classes: {list(parsed_rgb_ranges.keys())}")
        ...
        for class_name in metadata["classes"].keys():
            ...
            file_paths = [
                ...
                if os.path.isfile(os.path.join(class_dir, filename)) and filename != "metadata.json"
            ]
        ...
        if not saved_files["training_set"] or not saved_files["test_set"]:
            return {"error": "No valid files were uploaded or split between training and test sets"}
        ...
        return {
            ...
            "dataset_summary": {
                ...
                "files_per_class": {cls: info["count"] for cls, info in metadata["classes"].items()},
                "total_files": metadata["total_files"],
                "training_files": {class_name: len(files) for class_name, files in saved_files["training_set"].items()},
                "test_files": {class_name: len(files) for class_name, files in saved_files["test_set"].items()},
            },
